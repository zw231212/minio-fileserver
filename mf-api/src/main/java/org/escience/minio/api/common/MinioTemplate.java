...
import org.escience.minio.api.common.vo.MinioItem;
...
public class MinioTemplate {
    ...
    public MinioTemplate(String endpoint, String accessKey, String secretKey,Boolean secure) {
        this.endpoint = endpoint;
        this.accessKey = accessKey;
        this.secretKey = secretKey;
        this.secure = secure;
    ...
    public ObjectStat statObject(String bucket, String name) throws InvalidPortException,
            InvalidEndpointException, IOException, InvalidKeyException,
            NoSuchAlgorithmException, InsufficientDataException,
            InternalException, NoResponseException, InvalidBucketNameException,
            XmlPullParserException, ErrorResponseException {
        return getMinioClient().statObject(bucket, name);

    }

    public InputStream getObject(String bucket, String name) throws InvalidPortException,
            InvalidEndpointException, IOException, InvalidKeyException,
            NoSuchAlgorithmException, InsufficientDataException,
            InvalidArgumentException, InternalException, NoResponseException,
            InvalidBucketNameException, XmlPullParserException,
    ...
    public Boolean bucketExists(String bucket){
        ...
        try {
            exists = getMinioClient().bucketExists(bucket);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return exists;
    ...
    public Boolean objectExists(String bucket, String name) {
        ...
        try {
            objectStat = getMinioClient().statObject(bucket, name);
        } catch (Exception e) {
            e.printStackTrace();
        }
